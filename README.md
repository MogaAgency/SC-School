...
شغّل [`supabase/003_quizzes.sql`](supabase/003_quizzes.sql) مرة واحدة (بعد 002). بيعمل جداول
`quizzes` و`quiz_questions` و`quiz_answers` و`quiz_attempts`، ودالة `submit_quiz` اللي بتصحّح
جوه قاعدة البيانات. الإجابات الصح في جدول منفصل الطالب ماعندوش صلاحية يقراه، فمش بتوصل للمتصفح
قبل ما يسلّم.

- من صفحة الكورس في اللوحة، أيقونة الاختبار جنب كل درس بتفتح صفحة الاختبار: اسم، نشر/إخفاء،
  وأسئلة اختيار من متعدد (٢ لـ ٤ اختيارات وإجابة واحدة صح).
- الطالب بيلاقي الاختبار في آخر الدرس، بيسلّم، وبيشوف نتيجته والإجابات الصح فورًا. الإعادة
  مسموحة وأحسن نتيجة هي اللي بتتحسب.
- زرار **نتائج الاختبارات** في صفحة الكورس بيعرض جدول كل الطلاب × كل الاختبارات، وصفحة كل طالب
  فيها نتايجه كلها.
